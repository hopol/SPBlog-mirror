#!/usr/bin/env bash
# 手动镜像 jkjoy/Simple-PHP-Blog 的 main 分支。
set -euo pipefail
...
git add upstream
git commit -m "chore: mirror Simple PHP Blog source $SOURCE_REF ($SHORT_COMMIT)"
git tag -a "mirror-source-$SHORT_COMMIT" -m "Mirror Simple PHP Blog source $SOURCE_REF ($SHORT_COMMIT)"
git push origin HEAD
git push origin "mirror-source-$SHORT_COMMIT"
echo "已同步 Simple PHP Blog $UPSTREAM_BRANCH：$UPSTREAM_COMMIT"
